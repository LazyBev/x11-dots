...
main() {
    ...
    # Copy configurations
    for config in .emacs.d dunst neofetch/bk fcitx5 mozc fish i3 nvim rofi omf Misc/picom.conf; do
        sudo cp -rpf "$HOME/dotfiles/$config" "$HOME/.config/$config"
    done
    ...
    sudo cp -rpf "$HOME/dotfiles/Misc/pacman.conf"
